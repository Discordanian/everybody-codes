...
func _run() -> void:
	...
	var node_name: String = "Day%02d" % day_number
	var script_day: String = "day: int = %d" % day_number
	...
	var script_file: FileAccess = FileAccess.open(script_src, FileAccess.READ)
	if script_file == null:
	...
	script_content = script_content.replace("day: int = 0", script_day)

	var script_out: FileAccess = FileAccess.open(script_dst, FileAccess.WRITE)
	script_out.store_string(script_content)
	script_out.close()

	var updated_scene: String = scene_content.replace("scripts/day_00.gd", script_dst)
	updated_scene = updated_scene.replace("Day00", node_name)
